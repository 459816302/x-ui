...
install_bbr() {
    ...
    bash <(curl -L -s https://raw.githubusercontent.com/459816302/across/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/459816302/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
